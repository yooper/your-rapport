...
export const discoveryPluginSchema = z.object({
  ...
  action: z.enum(["CreateTab", "SubmitForm", "ForegroundRunner", "BackgroundRunner"]).default('CreateTab'),
  homePage: z.url().nullable(),
  description: z.preprocess((v) => (v === "" ? null : v), z.string().max(5000).nullable()).default(null),
  label: z.preprocess((v) => (v === "" ? null : v), z.string().min(1).max(255)),
  readOnly: z.boolean().default(false),
  sortOrder: z.number().int().min(0).default(0),
  timeOut: z.number().int().min(0).max(300000).default(3000),       // 0..5 minutes; adjust to your needs
  lastAccessedOn: nullableDate.default(null),
  ...
  version: versionString.default('0.0.1'),
  mimeTypeRegex: z.string().nullable().default(null),
  status: z.string().nullable().default(null),
  statusError: z.preprocess((v) => (v === "" ? null : v), z.string().nullable()).default(null),
  contentTypeHeader: z.enum(["", "application/json", "multipart/form-data", "application/octet-stream", "application/x-www-form-urlencoded"]).nullable().default(null),
