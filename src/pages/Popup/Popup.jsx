...
function LargeButtonGrid() {
  const buttons = [
    { title: 'Autoscroll Collect', onClick: () =>
      {
        chrome.runtime.sendMessage({
            cmd: "autoscrollCollect",
        }).then((response) => {
            processNotification({title: 'Autoscroll Collect Started', message: `Autoscroll collect has started. Press this button, again or press Crtl+Shift+Z to stop autoscroll. It will stop when it hits the bottom.`, type: 'success'});
        });
      }
    },
    { title: 'Single Collect', onClick: () =>
      {
        chrome.runtime.sendMessage({
            cmd: "popupSingleCollect",
        }).then((response) => {
            processNotification({title: 'Single Collect', message: `Single screenshot collected.`, type: 'success'});
        });
      }
    },
