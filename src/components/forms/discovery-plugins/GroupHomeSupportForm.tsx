      ...
      <FormGroup>
        ...
        <FormControl>
          <Autocomplete<Country, false, false, false>
            ...
            options={COUNTRIES}
            autoHighlight
            value={COUNTRIES.find(c => c.code === record.country) ?? null}
            onChange={(_, newValue) => {
              setRecord(prev => ({ ...prev, country: newValue?.code ?? null }));
            }}
            ...
            renderInput={(params) => (
              <StyledTextFieldNoWidth sx={{ width: 500 }} {...params} label="Select a country"
                InputProps={{
                  ...params.InputProps,
                  endAdornment: (
                    <>
                      <InputAdornment position="end" sx={{ mr: 0.5 }}>
                        <HelperPopover message="Select the country this discovery plugin is applicable for." />
                      </InputAdornment>
                      {params.InputProps.endAdornment}
                    </>
                  ),
