...
import {
  Avatar, Chip, Dialog, DialogActions, DialogContent, DialogContentText,
  DialogTitle, IconButton, List, ListItem, ListItemIcon, ListItemText,
  Typography
} from '@mui/material';
import {
  Apps as AppsIcon,
...
const DiscoveryPluginDialog: React.FC<DiscoveryPluginDialogProps> = ({
  ...
  const getIcon = (pluginType: string): JSX.Element => {
    return iconMap[pluginType] || (
      <Avatar><QuestionMarkIcon sx={{ fontSize: 'small' }} /></Avatar>
    );
  };
  ...
  const handleDelete = async (value: string) => {
    showLoader();
    await fetch_wrapper(`{{WebHost}}v1/selector?Name=${value}`, { method: 'DELETE' });
    hideLoader();
  };

  const selectorTypeKeys = Object.keys(getSelectorTypeMap());
  const groupNames = Array.from(new Set(plugins?.map((p) => p.groupName))).sort();
    ...
    <Fragment>
      {
        uxType === 'chip' ? (
          <Chip
            ...
            size="small"
            onDelete={selectorTypeKeys.includes(title) ? async () => { await handleDelete(selectorValue) } : undefined}
          />
        ) : (
          <IconButton
            key={`${title}-${uxType}`}
            ...
            <div>
              ...
              {groupNames.map((groupName) => (
                <div key={`group-${groupName}`}>
                  <h4>{groupName}</h4>
                  <List dense>
                    {plugins.filter(p => p.groupName === groupName).map((plugin) => (
                      <ListItem
                        dense
                        key={`plugin-${plugin.pluginType}-${plugin.uuid}`}
                        className="clickable"
                        onClick={() => discoveryPluginParser(plugin, record, selectorValue)}
                      >
                        <ListItemIcon><OpenInNewIcon /></ListItemIcon>
