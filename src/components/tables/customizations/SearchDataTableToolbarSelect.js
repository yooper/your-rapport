      ...
      <Fragment>
        <span style={{justifyContent: 'right'}}>
          <Tooltip title={'Merge multiple screenshots and download the image.'}>
            <IconButton >
              <BurstModeIcon onClick={async() => {
                showLoader()
                const orderedIndex = selectedRows.data.map(sr => sr.dataIndex);
                ...
                await downloadBase64Image(dataUri, 'your_rapport_merged_images.png');
                hideLoader()
              }}/>
            </IconButton>
          ...
          <Tooltip title={"Export Selected Rapports in a JSON file to share or backup."}>
            <IconButton>
              <CloudDownloadIcon onClick={async() => {
                const rapports = getRapportRecords(selectedRows, displayData, columns);
